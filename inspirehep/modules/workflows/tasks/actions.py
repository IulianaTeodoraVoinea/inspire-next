...
def validate_record(schema):
    """Check if the record is schema compliant and stop the workflow in ERROR state if it is not."""
    @with_debug_logging
    @wraps(validate_record)
    def _validate_record(obj, eng):
        validate(obj.data, schema)

        _validate_record.__doc__ = (
            'Validate the workflow record against the "%s" schema.'
            % schema
        )
